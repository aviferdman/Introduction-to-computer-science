...
public class BinaryNode<T> {
    ...
    public String toString() {
    	//send to another function which also counts the depth of the roots 
    	return stringWithSpaces(this,0);
    }
    
    private String stringWithSpaces (BinaryNode <T> root,int depth) {
    	//Initialize the value of the string
    	String s= "";
    	//if there is a left son send it to the recursion with depth +1
    	if(root.left!=null)
    		s=s+stringWithSpaces(root.left,depth+1);
    	//add to the string the appropriate number of spaces
    	for(int i=0;i<depth;i=i+1) {
    		s=s+"  ";
    	}
    	//add to the string the data of the node and go for the next row
    	s=s+root.data.toString()+"\n";
    	//if there is a right son send it to the recursion with depth +1
    	if(root.right!=null)
    		s=s+stringWithSpaces(root.right,depth+1);
    	return s;
    }
